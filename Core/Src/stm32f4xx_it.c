...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
extern uint8_t vision_rx_buf[];
extern void Vision_Decode_Callback(uint8_t *buf, uint16_t len);
...
void UART8_IRQHandler(void)
{
  ...
  if (__HAL_UART_GET_FLAG(&huart8, UART_FLAG_IDLE) != RESET) {
      ...
      __HAL_UART_CLEAR_IDLEFLAG(&huart8); // 必须先清除空闲中断标志位
      HAL_UART_DMAStop(&huart8);          // 暂停 DMA 接收，防止处理时数据被覆盖

      // 计算本次接收到的真实数据长度
      // 长度 = 缓冲区总大小 - DMA剩余未传输的数据量
      uint16_t rx_len = sizeof(uart8_rx_buf) - __HAL_DMA_GET_COUNTER(&hdma_uart8_rx);

      // 调用数据解析函数
      UART8_Parse_Data(uart8_rx_buf, rx_len);
